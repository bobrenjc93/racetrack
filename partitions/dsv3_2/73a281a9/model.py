...
MODEL_NAME = "dsv3_2"
PARTITION_NOTES = (
    "Deep-fusion partition: 5 fused kernels covering the full attention "
    "and MoE pipeline.  fused_attn_norm_qkv absorbs the pre-attention "
    "RMSNorm + QKV projection + split.  fused_q_proj_rope and "
    "fused_kv_proj_assemble each absorb a norm, projection, RoPE, and "
    "reassembly step.  Uses SDPA for causal prefill attention, and carries "
    "fused_residual_norm over from the 3336cdbd partition.  Routes every "
    "MoE expert through fused_swiglu so the existing Triton/Helion/CuteDSL "
    "activation kernels are used on the hot path."
)
...
def fused_attn_norm_qkv(
    ...
    *,
    ...
    x = rms_norm(hidden_states, norm_weight, eps)
...
def causal_attention(
    ...
    *,
    ...
    del causal_mask
    q_bh = q.transpose(0, 1).unsqueeze(0)
    k_bh = k.transpose(0, 1).unsqueeze(0)
    v_bh = v.transpose(0, 1).unsqueeze(0)
...
class SwiGLUExpert(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.dispatcher is None:
            activated = fused_swiglu(gate, up)
        else:
            activated = self.dispatcher.call("fused_swiglu", fused_swiglu, gate, up)
...
class RoutedMoE(nn.Module):
    ...
    def forward(
        ...
        expert_weights.scatter_(1, topk_ids, topk_weights)
...
class FlattenedMLAAttention(nn.Module):
    ...
    def forward(
        ...
        if self.dispatcher is None:
            q_c, kv_c, k_pe = fused_attn_norm_qkv(
                hidden_states, attn_norm_weight,
            ...
            q_c, kv_c, k_pe = self.dispatcher.call(
                "fused_attn_norm_qkv", fused_attn_norm_qkv,
                hidden_states, attn_norm_weight,
                self.fused_qkv_a_proj.weight,
                eps=config.rms_norm_eps,
                q_lora_rank=config.q_lora_rank,
...
class FlattenedDeepSeekBlock(nn.Module):
    ...
    def forward(
        ...
        if self.config.hc_mult <= 1:
            ...
            attn_out = self.attn(hidden_states, positions, self.attn_norm.weight)
            if self.dispatcher is None:
        ...
        x = self.attn(x, positions, self.attn_norm.weight)
