...
def _triton_rope(x, positions, rope_base):
    x_c = x.contiguous()
...
def fused_kv_proj_assemble(
    kv_c, kv_norm_weight, kv_b_weight, k_pe, positions,
    *, eps, num_heads, nope_dim, v_head_dim, rope_base, fallback,
):
    del fallback
    kv_c = _triton_rms_norm(kv_c, kv_norm_weight, eps)
    k_pe = _triton_rope(k_pe, positions, rope_base)
    tokens = kv_c.shape[0]
