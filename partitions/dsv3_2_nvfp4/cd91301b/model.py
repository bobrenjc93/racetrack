...
PARTITION_NOTES = (
    "Prefill-specialized hill-climb candidate. It keeps the ad727876 "
    "decode/indexer path for non-prefill calls, but for start_pos=0 "
    "prefill it skips the indexer projections and indexer cache build "
    "because the top-k mask covers every key. The prefill attention body "
    "uses torch scaled_dot_product_attention. This variant also dispatches "
    "the post-attention residual RMSNorm and SwiGLU activation. It further "
    "collapses prefill Q/KV B projections, RoPE, and cache writes into one "
    "dispatch op. This hill climb makes SDPA inputs contiguous to select the "
    "flash path and uses a scatter-built bf16 MoE routing matrix to reduce "
    "expert weighting overhead."
)
...
def fused_residual_norm(
    ...
    *,
    ...
    var = hidden.square().mean(dim=-1, keepdim=True)
    normed = hidden * torch.rsqrt(var + eps)
    return (weight.float() * normed).to(x.dtype), hidden.to(x.dtype)
...
def fused_attn_norm_qkv_prefill(
    ...
    *,
    ...
    if residual is None:
        ...
        hidden = x.float() + residual.float()
        residual_out = hidden.to(x.dtype)
    scale = torch.rsqrt(hidden.square().mean(dim=-1, keepdim=True) + eps)
    normed_x = (hidden * scale * norm_weight.float()).to(x.dtype)
    qkv = F.linear(normed_x, torch.cat([wq_a_weight, wkv_a_weight], dim=0))
    q_c = qkv[..., :q_lora_rank]
    kv_c = qkv[..., q_lora_rank:q_lora_rank + kv_lora_rank]
    k_pe = qkv[..., q_lora_rank + kv_lora_rank:]
...
class FlattenedMLAAttention(nn.Module):
    ...
    def forward(
        ...
        if mask is not None and start_pos == 0 and indexer_k is None and indexer_w is None:
            q_nope, q_pe, kv_expanded, k_pe_roped = _call(
                "fused_prefill_qkv_b_rope_cache", fused_prefill_qkv_b_rope_cache,
                q_c, self.q_norm.weight, self.wq_b.weight,
                kv_c, self.kv_norm.weight, self.wkv_b.weight,
                k_pe, freqs_cis,
            ...
            k = torch.cat([k_nope, k_pe_roped.unsqueeze(2).expand(-1, -1, self.n_heads, -1)], dim=-1)
        ...
        assert indexer_k is not None and indexer_w is not None
        qr = _call(
...
class SwiGLUExpert(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.dispatcher is None:
            ...
            hidden = self.dispatcher.call("fused_swiglu", fused_swiglu, gate, up)
...
class RoutedMoE(nn.Module):
    def __init__(
        ...
        self.experts = nn.ModuleList([
            SwiGLUExpert(config.hidden_size, config.moe_intermediate_size, dispatcher)
            for _ in range(config.n_routed_experts)
        ])
        shared_intermediate = config.moe_intermediate_size * config.n_shared_experts
        self.shared = (
            SwiGLUExpert(config.hidden_size, shared_intermediate, dispatcher)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        expert_weights = torch.zeros(
            (x.shape[0], self.config.n_routed_experts),
            device=x.device,
            dtype=x.dtype,
        )
        expert_weights.scatter_(1, indices, weights)
        y = torch.zeros_like(x)
        for expert_id, expert in enumerate(self.experts):
            weight = expert_weights[:, expert_id : expert_id + 1]
            y.addcmul_(expert(x), weight)
...
class FlattenedDeepSeekBlock(nn.Module):
    def __init__(self, config: DeepSeekNVFP4Config, dispatcher: KernelDispatcher | None = None):
        ...
        self.ffn = RoutedMoE(config, dispatcher)
    ...
    def forward(
        ...
        if mask is not None and start_pos == 0:
            residual_out, q_c, kv_c, k_pe = _call(
                "fused_attn_norm_qkv_prefill", fused_attn_norm_qkv_prefill,
        ...
        x, residual = _call(
            ...
            x, residual_out, self.ffn_norm.weight,
            eps=config.rms_norm_eps,
        )
        x = self.ffn(x)
        return x, residual
