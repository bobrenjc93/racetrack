...
PARTITION_NOTES = (
    ...
    "the post-attention residual RMSNorm and SwiGLU activation."
)
...
class FlattenedMLAAttention(nn.Module):
    ...
    def forward(
        ...
        if mask is not None and start_pos == 0 and indexer_k is None and indexer_w is None:
            qr = _call(
                "fused_norms_rope_cache_prefill", fused_norms_rope_cache_prefill,
            ...
            q_nope, q_pe = _call(
                "fused_q_prefill_proj", fused_q_prefill_proj,
                qr, self.wq_b.weight,
                n_heads=self.n_heads, qk_head_dim=self.qk_head_dim,
                qk_nope_head_dim=self.qk_nope_head_dim,
            )
            q_pe = _call(
                "fused_q_rope_prefill", fused_q_rope_prefill,
                q_pe, freqs_cis,
            )

            q = torch.cat([q_nope, q_pe], dim=-1)
            kv_c_normed = self.kv_cache[:bsz, :end_pos]
            k_pe_roped = self.pe_cache[:bsz, :end_pos].unsqueeze(2)
