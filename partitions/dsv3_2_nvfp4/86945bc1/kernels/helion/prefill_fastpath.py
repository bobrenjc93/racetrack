...
def _residual_rms_norm(x, residual, weight, eps):
    ...
    hidden_out = hidden.to(x.dtype)
    variance = hidden.pow(2).mean(-1, keepdim=True)
    normed = (weight * (hidden * torch.rsqrt(variance + eps))).to(x.dtype)
    return hidden_out, normed
...
def fused_norms_rope_cache_prefill(
    ...
    *, eps, start_pos, fallback,
    ...
    del fallback
    import importlib
    apply_rotary_emb = importlib.import_module("partitions.dsv3_2_nvfp4.86945bc1.model").apply_rotary_emb
