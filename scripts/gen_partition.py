...
into a single kernel. The codegen emits a declarative spec.py plus per-backend
kernel stubs; the baseline model.py is never patched. At runtime the fused ops
are applied by the registered FX-pattern matchers and pre-trace patchers, so a
fusion only takes effect if its op has a matching matcher/patcher.
...
# Module-patch ops that have a registered patcher in racetrack/pre_trace.py.
# Kept in sync by hand (gen_partition.py intentionally avoids importing torch,
# which the live _REGISTRY pulls in). Any module_patch op missing here would be
# silently skipped by apply_pre_trace_patches at runtime, so we refuse to emit
# it rather than write a spec.py that claims an inert fusion.
_MODULE_PATCH_OPS = {
    ...
    "fused_q_rope_quant",
    "fused_prefill_qkv_b_rope_cache",
    "fused_attn_norm_qkv_prefill",
    "fused_norms_rope_cache_prefill",
    "fused_q_prefill_proj",
    "fused_q_rope_prefill",
}
...
def _classify_op_kind(op_name: str) -> str:
    ...
    if op_name in _PRE_TRACE_OPS:
        return "pre_trace"
    if op_name in _MODULE_PATCH_OPS:
        return "module_patch"
    print(
        f"ERROR: Fusion '{op_name}' has no FX-pattern matcher and no registered "
        f"pre-trace/module patcher, so it would run as the unfused baseline.\n"
        f"Add a matcher in racetrack/fx_patterns.py or a patcher in "
        f"racetrack/pre_trace.py (and register the op name in the corresponding "
        f"set in this file) before generating a partition for it.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def main() -> None:
    ...
    if no_recipe:
        print(
            "\nNote: no recipe for these fusions — they take effect only if the "
            "op has a runtime matcher/patcher (verified during codegen):"
        )
        for f in no_recipe:
            print(f"  {f.name}")
